<?php  
 class Rest {  
   ...
   public function mostrarRespuesta($data, $estado) {  
     $this->_codEstado = ($estado) ? $estado : 200;//si no se env�a $estado por defecto ser� 200  
     $this->setCabecera();  
     echo $data;  
     exit;  
   }  
   ...
   private function limpiarEntrada($data) {  
     ...
     if (is_array($data)) {  
       ...
       if (get_magic_quotes_gpc()) {  
         ...
         //Cuando esta extensi�n est� activada, PHP a�ade autom�ticamente caracteres de escape (\) delante de las comillas que se escriban en un campo de formulario.   
