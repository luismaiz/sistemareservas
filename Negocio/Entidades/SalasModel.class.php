 ...
 * @package entity
 */
class SalasModel extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='SalasModel';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='salas';
	const SQL_INSERT='INSERT INTO `salas` (`idSala`,`NombreSala`,`CapacidadSala`,`DescripcionSala`,`FechaAlta`,`FechaBaja`) VALUES (?,?,?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `salas` (`NombreSala`,`CapacidadSala`,`DescripcionSala`,`FechaAlta`,`FechaBaja`) VALUES (?,?,?,?,?)';
	const SQL_UPDATE='UPDATE `salas` SET `idSala`=?,`NombreSala`=?,`CapacidadSala`=?,`DescripcionSala`=?,`FechaAlta`=?,`FechaBaja`=? WHERE `idSala`=?';
	const SQL_SELECT_PK='SELECT * FROM `salas` WHERE `idSala`=?';
	const SQL_DELETE_PK='DELETE FROM `salas` WHERE `idSala`=?';
	const FIELD_IDSALA=12179228;
	const FIELD_NOMBRESALA=991178730;
	const FIELD_CAPACIDADSALA=-1550408105;
	const FIELD_DESCRIPCIONSALA=-1978261998;
	const FIELD_FECHAALTA=-157075209;
	const FIELD_FECHABAJA=-157056299;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of SalasModel instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param SalasModel $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return SalasModel[]
	 */
	public static function findByExample(PDO $db,SalasModel $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of SalasModel instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	/**
	 * Will execute the passed statement and return the result as an array of SalasModel instances
	 *
	...
	/**
	 * returns the result as an array of SalasModel instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return SalasModel[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of SalasModel instances.
	 *
	...
	/**
	 * Fetch ClasesModel's which this SalasModel references.
	 * `salas`.`idSala` -> `clases`.`idSala`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	 * @return ClasesModel[]
	 */
	public function fetchClasesModelCollection(PDO $db, $sort=null) {
		$filter=array(ClasesModel::FIELD_IDSALA=>$this->getIdSala());
		return ClasesModel::findByFilter($db, $filter, true, $sort);
	}
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'SalasModel');
	}

	/**
	 * get single SalasModel instance from a DOMElement
	 *
	...
	/**
	 * get all instances of SalasModel from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('SalasModel') as $node) {
